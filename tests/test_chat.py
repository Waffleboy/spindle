...
class TestStructuredRetrieval:
    ...
    async def test_result_structure(self, seeded_db):
        ...
        for r in taxonomy_results:
            ...
            assert "is_approximate_date" in r
            assert "document_date" in r

    @pytest.mark.asyncio
    async def test_is_approximate_date_true_when_no_report_date(self, seeded_db):
        """Documents without report_date should have is_approximate_date=True."""
        results = await structured_search("revenue", QueryType.FACT_LOOKUP, seeded_db)
        taxonomy_results = [r for r in results if r.get("type") != "contradiction"]
        # Default seeded docs have no report_date set, so all should be approximate
        for r in taxonomy_results:
            assert r["is_approximate_date"] is True

    @pytest.mark.asyncio
    async def test_is_approximate_date_false_when_report_date_set(self, db_session):
        """Documents with report_date should have is_approximate_date=False."""
        doc = Document(
            id="doc-rd",
            original_filename="dated_report.pdf",
            storage_path="data/originals/dated_report.pdf",
            file_type="pdf",
        ...
        taxonomy = TaxonomySchema(
            id="tax-rd",
            corpus_id="corpus-rd",
            dimensions=[{"name": "metric", "description": "Test", "expected_type": "number"}],
            doc_type="Test",
        )
        db_session.add(taxonomy)
        db_session.commit()
        ...
        results = await structured_search("metric", QueryType.FACT_LOOKUP, db_session)
        taxonomy_results = [r for r in results if r.get("type") != "contradiction"]
        assert len(taxonomy_results) == 1
        assert taxonomy_results[0]["is_approximate_date"] is False
        # Should use report_date, not uploaded_at
        assert "2024-06-15" in taxonomy_results[0]["document_date"]

    @pytest.mark.asyncio
    async def test_temporal_sorting_prefers_report_date(self, db_session):
        """Temporal sorting should use report_date when available, falling back to uploaded_at."""
        # doc-old has older uploaded_at but newer report_date
        doc_old = Document(
            id="doc-old",
            original_filename="old_upload.pdf",
            storage_path="data/originals/old_upload.pdf",
            file_type="pdf",
            report_date=datetime(2024, 12, 1, tzinfo=timezone.utc),
            uploaded_at=datetime(2024, 1, 1, tzinfo=timezone.utc),
        )
        # doc-new has newer uploaded_at but no report_date
        doc_new = Document(
            id="doc-new",
            original_filename="new_upload.pdf",
            storage_path="data/originals/new_upload.pdf",
            file_type="pdf",
            report_date=None,
        ...
        taxonomy = TaxonomySchema(
            id="tax-sort",
            corpus_id="corpus-sort",
            dimensions=[{"name": "val", "description": "Test", "expected_type": "number"}],
            doc_type="Test",
        )
        db_session.add(taxonomy)
        db_session.commit()
        ...
        results = await structured_search("val", QueryType.TEMPORAL, db_session)
        taxonomy_results = [r for r in results if r.get("type") != "contradiction"]
        assert len(taxonomy_results) == 2
        # doc-old has report_date=2024-12-01, doc-new has uploaded_at=2024-06-01
        # So doc-old should come first (newer effective date)
        assert taxonomy_results[0]["document"] == "old_upload.pdf"
        assert taxonomy_results[1]["document"] == "new_upload.pdf"

    @pytest.mark.asyncio
    async def test_contradiction_temporal_context(self, seeded_db):
        """Contradictions should include temporal_context field."""
        results = await structured_search("revenue", QueryType.FACT_LOOKUP, seeded_db)
        contradictions = [r for r in results if r.get("type") == "contradiction"]
        assert len(contradictions) == 1
        assert "temporal_context" in contradictions[0]
        # Both docs have uploaded_at set, so temporal_context should be present
        assert contradictions[0]["temporal_context"] is not None
        assert "Most recent value" in contradictions[0]["temporal_context"]
